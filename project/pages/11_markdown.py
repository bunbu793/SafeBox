...
.scene{width:100vw;height:100vh;display:flex; justify-content:center;align-items:center;perspective:900px;transform: translateY(-170px); /* ← ここで調整できる */}
...
width:100px;height:100px;border-radius:50%;border:12px solid #00ff88;background:transparent;
...
@keyframes spinIn{0%{transform:scale(0) rotateY(0deg);opacity:0;}
...
@keyframes spread{0%{transform:translate(-50%,-50%) scale(0.3);opacity:1;}
100%{transform:translate(var(--x), var(--y)) scale(1.8);opacity:0;}}
@keyframes vanish{0%{transform:scale(1) translateY(0);opacity:1;filter:blur(0px);}
...
<style>
body{margin:0;background:white;overflow:hidden;}
.scene{width:100vw;height:100vh;display:flex;justify-content:center;align-items:center;perspective:1000px;transform: translateY(-170px); /* ← ここで調整できる */}
.totem{position:relative;width:160px;height:160px;transform-style:preserve-3d;
animation:spinIn 3.2s ease-out,float 4s ease-in-out infinite 3.2s,vanish 1.6s ease-in-out 7.0s forwards;}
.core{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);
width:100px;height:100px;}
.core::before,.core::after{content:"";position:absolute;left:50%;top:50%;width:100px;height:14px;
background:#ff2b2b;box-shadow:0 0 20px #ff2b2b,0 0 40px #ff7b00,0 0 70px rgba(255,120,0,.9);
